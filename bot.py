...
# Конфигурация (TOKEN будет браться из переменных окружения)
CHANNEL_ID = "@ulx_ukraine" # Измените на ваш ID канала, если он отличается
...
"""
    ...
    try:
        if update.message: # Проверяем, что это сообщение, а не callback
            await update.message.reply_text(
                welcome_text,
                reply_markup=InlineKeyboardMarkup(keyboard),
                parse_mode=ParseMode.HTML
            )
        elif update.callback_query: # Если это callback из другого меню, редактируем сообщение
...
async def start_selling(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    else: # Это CommandHandler /start или /sell
    ...
    await message_to_edit.reply_text( # Используем reply_text для нового сообщения
...
"""
    ...
    else: # Если это вызвано из MessageHandler (например, после добавления последнего фото)
        await update.message.reply_text(
            preview_text,
            reply_markup=InlineKeyboardMarkup(keyboard),
            parse_mode=ParseMode.HTML
...
def main() -> None:
    ...
    # Логика для запуска на Render с вебхуками или локально с polling
